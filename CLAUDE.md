...
`docs/proposals/` (the `PROPOSAL_*` design threads, **six concluded and one live** — a live one wins over the
roadmap files until its items land, and `PROPOSAL_0_7` stopped being live when the twelfth landed; **the live one is
`PROPOSAL_0_7_PT2`, RM163–RM168, and it is drafted rather than decided, so it is the one live thread that does NOT
yet win over ROADMAP**.
