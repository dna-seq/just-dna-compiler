...
- **`docs/proposals/`** — the `PROPOSAL_*` design threads, **nine concluded, none live**. A live one
  ...
  its items' ROADMAP_HISTORY entries, never alone** — entries there say things their own probe or
  build later refuted, so an unprobed entry is a question and a probed one is still only a proposal.
  An item still open is tracked in ROADMAP like any other open item, never by the proposal. A dated
  addendum is where a departure from a build order is recorded rather than contradicted silently.
- **`docs/probes/`** — the measured rounds, one topic each: `CIVIC_IDENTITY_PROTOCOL` (resolving a
  variant a source names but does not identify) with its source-agnostic handout
  `IDENTITY_FROM_A_NAME` and `CIVIC_LEGACY_INSERTIONS`; `rm170_kleene` (RM170's design record — the
  advocacy the shipped check follows, and the two repairs it refuses); `CONTRADICTION_CORPORA`
  (**read before designing anything that reads a source disagreeing with itself**); `MITOMAP_STATUS`
  (its `status` column is a **two-token grammar**); `rm171_diff_strategy` (**read before touching the
  MITOMAP lanes** — the increment against ClinVar, never the photocopies, and why `VUS*` is withheld);
...
| [RM_TOC.md](docs/RM_TOC.md) | where any `RMn` lives, status included — the complete list, **paged by number** (`RM_TOC_000_099.md`, …) behind one entry page that holds the reservations. **Claim a new number with `.claude/rm-next.py`** (scan + locked reservation), never by reading the highest off the file — an index is not an allocator, and two sessions once filed the same RM159 | `grep -n 'RM47' docs/RM_TOC*.md` |
| [ROADMAP.md](docs/ROADMAP.md) | open items, the idea-book, and **three** trackers: reserved-namespace, 1.0-cleanup, and the **parking-condition gate audit** (a deferral's gate is dishonest when nothing can satisfy it until the item ships — check a new parked item against it at filing) | `grep -n '^## RM' docs/ROADMAP.md` |
...
| [CHANGELOG.md](docs/CHANGELOG.md) | what shipped, newest first (shared across the ecosystem repos); **the 0.7 line onward**, with the 0.6 entries in [history/](docs/history/CHANGELOG_0_6.md) and pre-0.6 in [history/](docs/history/CHANGELOG_PRE_0_6.md). Both cuts are at a **tag**, never a date — grep across all three halves | `grep -rn '^## 2026-' docs/CHANGELOG.md docs/history/` |
| [INTEGRATION_0_6.md](docs/history/INTEGRATION_0_6.md) | the 0.5.4→0.6 surface delta and the per-consumer check/change lists. **Archived to `docs/history/` at the 0.7.0 cut** — a superseded upgrade note is a closed record, and the live one is the row above | `grep -n '^## \|^### ' docs/history/INTEGRATION_0_6.md` |
...
| [index.md](docs/index.md), [GETTING_STARTED.md](docs/GETTING_STARTED.md), [CONSUMING.md](docs/CONSUMING.md), [COMPILING.md](docs/COMPILING.md), [ENRICHING.md](docs/ENRICHING.md) | the **user-facing** front of the docs site: a landing page and one task guide per audience (consumer / compiler operator / enricher operator), sitting *in front of* the tier references and citing them. Every command in them was run before it was written. They add no rule — a rule they seem to state lives in the reference, and the guide is wrong if the two differ | `grep -rn '^## ' docs/GETTING_STARTED.md docs/CONSUMING.md docs/COMPILING.md docs/ENRICHING.md` |
| [TABLES.md](docs/TABLES.md) | the **hand-written half** of the per-table reference — one `## <name>.csv` section per table: the grain of a row, which cells a person decides, why the artifact has its shape, the symptom when it is wrong. **A build input, not a page**: sections are spliced above the *generated* columns on each table's site page, so never restate a column here. Required for every lead table, optional for the rest, asserted asymmetrically | `grep -n '^## ' docs/TABLES.md` |
...
| [DRAFTING_MATURITY.md](docs/DRAFTING_MATURITY.md) | the post-RM228 read on drafting as a **mechanism**: the four patterns the seven providers taught (a copy drifts toward its author's mental model; the correct-looking implementation is where the subtle bug lives; a justification narrower than its scope becomes a blind spot; an import cycle is a diagnosis) and the six things still missing, largest first — **no common `DraftResult` protocol, so `covered` is still hand-written seven times**. Read before adding a provider or unifying one | `grep -n '^## ' docs/DRAFTING_MATURITY.md` |
| [CACHE_SURFACE.md](docs/CACHE_SURFACE.md) | what a `CACHE_LANES` entry is, the three stages and the reason an absent one must state, and **a checklist for adding a lane where every line names the test that asserts it**. Written after a lane shipped with a `publish_repo` no command could reach (RM202) | `grep -n '^- \[ \]' docs/CACHE_SURFACE.md` |
...
| [RELEASE_CYCLE.md](docs/RELEASE_CYCLE.md) | how an `RMn` becomes a release: the seats, **version by omission** (non-blocked patch items go into the next patch on `main` unasked; an undecided part gets an interview first; **a commit on `main` is patch scope only** — minor-class work goes on the open minor branch, `0.8` today), the minor cycle from interview to cut, and tic/tac **after 1.0 only**. The §4 item counts are a lamp, never a start pistol | `grep -n '^## ' docs/RELEASE_CYCLE.md` |
...
| [CONSUMER_SUGGESTIONS_HISTORY.md](docs/CONSUMER_SUGGESTIONS_HISTORY.md) | answered `Sn`, verbatim, with the reply; it keeps the **whole** contents list across all three halves. **S62 onward live here**; the 0.6 line's S25–S61 sit in [history/](docs/history/CONSUMER_SUGGESTIONS_HISTORY_0_6.md) and 0.5's S1–S24, S27, S28 in [history/](docs/history/CONSUMER_SUGGESTIONS_HISTORY_PRE_0_6.md) | `grep -rn '^## S[0-9]' docs/CONSUMER_SUGGESTIONS_HISTORY.md docs/history/` |
...
| `docs/proposals/PROPOSAL_*.md` — **enumerate with `ls`**, a hand-kept list here goes stale | design threads with their charter checks and open questions. **0.6 has two and 0.7 has four** — a `PT2`, `PT3`, `PT4` sort the items that landed behind each preceding round, so the count grows within a release and the sentence stating it goes stale; `ls` is the answer, as the first column already says. They are records, not plans; how to read one is the `docs/proposals/` bullet above | `grep -n '^## ' docs/proposals/PROPOSAL_0_7_PT2.md` |
...
| [audit/](docs/audit/README.md) | the **2026-09-11** code-first re-derivation of all three tier references (the 2026-08-18 round is at `0e5fc58^`), and the instrument that found RM93–RM100 and then RM207–RM209. Its durable material is **merged**; what stays there is dated detail (per-parquet columns, per-command flags) that would rot in a maintained doc. Never edit one to record a fact | `grep -rn '<symbol>' docs/audit/` |
...
- Either spelling is a key: a map keyed on one spelling answers the other as a table it never heard of. `@sidecar-name-and-place`
...
- Kill a background job by **pgid**, never `pkill -f`/`-x` on a name: two sessions share this tree. `@two-agents-in-one-tree-cannot-use-name-matched-process-cleanup`
...
Once an item is an `RMn`, [RELEASE_CYCLE.md](docs/RELEASE_CYCLE.md) says which release takes it and how
that release gets built and cut.
...
- **`features/` is the scenario corpus (RM149, drafted 2026-09-13 and unreviewed)** — Gherkin derived
  **from** the code, with no step definitions and no runner in any tier. It is at the root rather than
  in `docs/` on purpose, so it does not publish and P3 does not reach it. The registries it walks are
  covered by set equality; a new warning code with no scenario fails `schema/tests/test_feature_corpus.py`.
