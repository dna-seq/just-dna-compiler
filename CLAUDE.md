...
`docs/proposals/` (the `PROPOSAL_*` design threads, **nine concluded, none live** — **`PROPOSAL_0_7_PT4` became a record on 2026-09-10**, the night it was drafted and built: adopting AlphaGenome's AVI artifact as RM191–RM195, with RM191/192/193 shipped, RM194 unblocked-but-unbuilt and RM195 open as a blocker, all four tracked in ROADMAP like any other open item rather than by the proposal. Its evidence base is `docs/probes/ALPHAGENOME_ATLAS.md`, eleven measured rounds, **four of whose own claims were refuted by later measurement** — every one a plausible reading of upstream prose that the bytes then contradicted, which is why that proposal's standing rule was measured-not-inferred. **The build round refuted four more, three of them the proposal's own**: losslessness is about the *decimal* and dividing the integer back disagrees on 53% of rows; the interval RPC's blocker was `Interval.strand` having no zero member rather than the field mask or 32 bp chunking it names; and a fourth that was **the build's own mistake rather than the proposal's** — a 43 GB artifact size that turned out to be the builder's `sink_parquet` fragmenting the delta encoding, where the proposal's 34.4 was right (RM197, and the CHANGELOG entry for it is the one to read). Read the proposal beside RM191–RM194's ROADMAP_HISTORY entries, never alone. Then — **`PROPOSAL_0_7_PT3` became a record on 2026-09-03**, when its third and last item landed: the build plan for RM174, RM160 and RM171, all three decided with the maintainer and none of them designed there. It carries **two** dated addenda — RM160's authored column pair, which its release-class line had priced at none, and RM171's five departures from the build order — a live one wins over the
