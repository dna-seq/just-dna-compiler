...
`docs/proposals/` (the `PROPOSAL_*` design threads, **seven concluded and one live** — **`PROPOSAL_0_7_PT3` is LIVE** as of 2026-09-03: the build plan for RM174, RM160 and RM171, all three decided with the maintainer and none of them designed there, so it wins over the roadmap files until its items land — a live one wins over the
...
| PROPOSAL_[0_4_1\|0_5\|0_5_1\|0_6\|0_6_PT2\|0_7\|0_7_PT2].md | design threads with their charter checks and open questions. **0.6 has two and so does 0.7** — a PT2 sorts the items that landed behind the first round. **0_7 is a record, not a plan**: twelve items decided per-item with the maintainer, every one shipped, entries in ROADMAP_HISTORY. Its **two dated addenda** (RM140 and RM152, both 2026-08-31) are decisions taken after the round closed, kept there because they have the round's shape and shipped inside the same uncut 0.7.0 — they are addenda rather than a thirteenth and a fourteenth item, and every bare "twelve" in that file means the 2026-08-27/28 round. **0_7_PT2 is a record too** (drafted and decided 2026-09-01): RM163–RM168, the six source-adoption items, each gated on a probe the entry had not run. Read it for the two-stage drift — **five of six entries say something their own probe contradicted, and then four of six drafted verdicts were overturned in the maintainer pass**, so an unprobed entry is a question and a probed one is still only a proposal. RM163/165/167/168 shipped the same day, RM164 parked (spinning off RM171), RM166 is sequenced last | `grep -n '^## ' docs/proposals/PROPOSAL_0_7_PT2.md` |
