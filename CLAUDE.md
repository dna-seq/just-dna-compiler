...
| `docs/proposals/PROPOSAL_*.md` — **enumerate with `ls`**, a hand-kept list here goes stale | design threads with their charter checks and open questions. **0.6 has two and so does 0.7** — a `PT2` sorts the items that landed behind the first round. They are records, not plans; how to read one is the `docs/proposals/` bullet above | `grep -n '^## ' docs/proposals/PROPOSAL_0_7_PT2.md` |
