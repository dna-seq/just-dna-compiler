...
`docs/proposals/` (the `PROPOSAL_*` design threads, **eight concluded and one live** — **`PROPOSAL_0_7_PT4` is the live one**, drafted 2026-09-10: adopting AlphaGenome's AVI artifact as RM191–RM195, and it wins over the roadmap files until its items land. Its evidence base is `docs/probes/ALPHAGENOME_ATLAS.md`, eleven measured rounds, **four of whose own claims were refuted by later measurement** — every one a plausible reading of upstream prose that the bytes then contradicted, which is why that proposal's standing rule is measured-not-inferred. Then — **`PROPOSAL_0_7_PT3` became a record on 2026-09-03**, when its third and last item landed: the build plan for RM174, RM160 and RM171, all three decided with the maintainer and none of them designed there. It carries **two** dated addenda — RM160's authored column pair, which its release-class line had priced at none, and RM171's five departures from the build order — a live one wins over the
