...
| **2026-09-11** | Top-down half: SCHEMAS' module map eight modules short; ENRICHER's twenty-six short with a duplicated row and two stale counts; the check table missing `variant_impact_agreement`, a vocabulary member whose own comment says it was audited against that table; no AlphaGenome section in the tier that produces `expression_effects.csv`; the licence table scoped to PGx where the registry holds eighteen sources; `INTEGRATION_0_7.md` contradicting itself on whether RM143 had shipped, and calling the shipped RM194 open. Bottom-up half: **RM207** (a both-modes-fatal refusal looking up the key an expansion had just rewritten, *and* living only on the compile side), **RM208** (an `except` on an ancestor of the retried type, so one client never retried and its mirror image never translated), **RM209** (the publish half walked a registry the pull half kept a copy of). Reconciling the two halves to the end took the round to **RM207–RM223**, all seventeen shipped — RM215 among them, after its 1.0 filing turned out to have sized the change by citing a parquet retype for something that retypes nothing — of ~30 candidates plus what the top-down half raised and what a consumer measured against the upgrade guide while it was open (RM223); one candidate refuted on reading (`gwas._get`'s bare re-raise, where the static guard written during the repair was the thing that was wrong); the rest recorded, not filed |
