...
Twenty-three names. `LEAD_PARQUETS` is the ten carrying a module's own annotation rows — `weights` plus the
...
| **partial rows (0.5)** | ✅ stubbed columns validated by omission; the stub itself never compiles | — (writes authored CSVs, not parquet) | ✅ added / already-present / invalid; `DraftReport.header_extended` names the columns the header grew by — exactly the ones the **written** rows fill, never one only a rejected or already-present row filled, and a raw blank is not a fill (2026-09-09); settled before the existing rows are re-rendered (2026-09-02: an append into a header that predates a column used to raise a bare `ValueError`). Both writers rewrite the author's file atomically, and since 2026-09-12 both take **`before_commit`** — a callback run after the bytes are down and before the rename, so a drafter's licence row lands inside the commit of the table it licenses rather than after it (RM232). Optional and defaulted, so a caller that passes nothing is unaffected; it fires only when the call actually writes | complete (`draft.PartialRow`, `append_partial_rows`) |
