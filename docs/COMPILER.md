...
| strict compile (0.4.1) | ✅ `compile_module(..., strict=True)` fails (before any parquet is written) if a `VariantRow` lacks a resolved `(chrom, start)` after resolution | — (refuses a partial artifact rather than emitting one) | — | complete (opt-in; default best-effort unchanged) |
| `content_signature` (0.4.1) | ✅ `integrity.content_signature` over raw authored data rows: normalized (`model_dump` json, `exclude_none`) + deterministically sorted, name-/Ensembl-independent | ✅ **manifest** `content_signature` (out of `artifact.digest`); `compiler.content_signature(spec_dir)` + `just-dna-compiler signature` compute it without recompiling | — | complete (canonical dedup identity; marketplace adopts) |
| CLI (0.4.1) | ✅ `just-dna-compiler` Typer app: `validate` / `compile` / `reverse` over the Python API; `--strict`, `--strip-identity`/`--authority-key`, `--ensembl-cache`, `--no-resolve`; exit 0/1 | — | — | complete (compiler-only dep; tiers intact) |
