...
- **One proof-of-work per call, so a pass collects its records and writes once.** `enrich()` writes all
  four of its checks at the end of the run, `enrich_literature` its three. The merge is what keeps
...
(reference allele, wrong build, clinical significance, rsID currency), `enrich_literature` attests
three (citation existence, identifier agreement, provenance quote) and `enrich_clinpgx` attests its
