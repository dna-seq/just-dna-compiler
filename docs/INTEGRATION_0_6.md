...
**Status: 0.6 is uncut.** All three `pyproject.toml` files read `0.6.0`, `git tag` stops at `v0.5.4`,
and nothing 0.6 is on PyPI. *"In the tree" means committed and nothing more* — check
[CHANGELOG.md](CHANGELOG.md) for whether a version was cut before promising a field to anyone. Read
this document as *prepare for*, not *upgrade to*.
