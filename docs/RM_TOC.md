...
- **[RM15](ROADMAP.md#rm15--build-agnostic-identity--multi-build-support-other-builds-support)** — build-agnostic identity & multi-build support (refget tables beyond GRCh38). Paired with the `end`-column item in the major bucket: both blocked on settling the coordinate convention. · *also in* USE_CASES, SCHEMAS, COMPILER, CHANGELOG, PROPOSAL_0_5
...
- **[RM31](ROADMAP.md#rm31--one-indel-spelled-two-ways-defeats-allele-aware-resolution)** — one indel spelled two ways defeats allele-aware resolution. ClinVar's `X:634689 CAG>C` and Ensembl's `X:634690 AGAG>AG` are the same 2 bp deletion; `genotype_fits` compares strings. · *also in* CLAUDE.md, CHANGELOG
...
- **[RM33](ROADMAP.md#rm33--source-names-two-different-things-in-two-tables)** — `source` names a *link* in `resolution.csv` and a *licensed source* in `sources.csv`; the compiler compares them by string equality. Both obvious repairs are wrong, and one is charter-illegal. · *also in* CLAUDE.md, CHANGELOG
