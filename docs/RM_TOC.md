...
- **[RM36](ROADMAP.md#rm36--a-model-property-cannot-know-its-modules-build)** — a model property cannot know its module's build: `HeteroplasmyRow.variant_key` mints a GRCh38 VA on a GRCh37 module. The one instance the 2026-08-06 build sweep left unfixed, because all three obvious repairs are design decisions; bounded today (not a model field, never reaches parquet). · *also in* CLAUDE.md, SCHEMAS, CHANGELOG
