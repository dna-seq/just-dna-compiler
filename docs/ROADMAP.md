...
**Probed and drafted in [PROPOSAL_0_7_PT2](proposals/PROPOSAL_0_7_PT2.md#rm164--heteroplasmycsv-is-a-shipped-table-kind-with-no-source-behind-it) on 2026-09-01 — proposed PARKS to 0.8, pending the maintainer pass.** The durable blocker is the **axis**, not the source: `tissue` is in `HeteroplasmyRow`'s key and the shipped module's content is *about* the tissue, while a per-variant pathogenicity table has no tissue axis and gnomAD's mtDNA callset is blood only — neither fills the columns the kind exists for, whatever their terms turn out to be. **MITOMAP is separately unreachable by the machinery that would consume it**: six paths, including `/robots.txt`, return Cloudflare's *managed JS challenge* — a 403 to `httpx`, live in a browser. That is bot mitigation and a fetch problem, **not a refusal and not unestablishable terms**, which a human settles in a browser in a minute; the enricher fetches with `httpx` and no builder solves a JS challenge, so the access question needs a human answer. Not closed — the axis argument is reasoning over one module's key, not a survey of the mtDNA literature.
