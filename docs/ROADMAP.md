...
| Compiler injected-reference (DuckDB) resolution path | Superseded in 0.5 by `resolve_from_table` (the `resolution.csv` path). The `ensembl_cache` DuckDB path + `resolver.py`'s duckdb lookups are kept as a working alias (P3), but the end state is *the compiler consumes only the table* — removing the path drops `duckdb` from the compiler entirely (pure-Python, self-contained). | Remove at 1.0 (moves the duckdb resolver fully into `just-dna-enricher`); until then it stays a working superseded path. |
