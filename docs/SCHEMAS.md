...
and the `source_field?` VCF pointer, plus **`pmid?` — the boundary citation added in 0.6 (RM47)**, a
free-form PubMed pointer under the same grammar as `StudyRow.pmid`. The bin row cites; the citation
table describes, which is what keeps `StudyRow`'s provenance column set from migrating here one column
at a time. Per-kind key fields: `ActivityPhenotypeRow`→`(gene)`;
