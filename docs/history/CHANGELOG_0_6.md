# Changelog — the 0.6 line

The closed half of [CHANGELOG.md](../CHANGELOG.md), split out on 2026-09-12 when 0.7.0 was cut and
published: every entry that shipped in a 0.6 number. Newest first, same as the live file.

**The boundary is the `v0.6.6` tag, not a date.** It was read off `git show v0.6.6:docs/CHANGELOG.md`
rather than inferred, because 0.6.6 was tagged at 00:46 on 2026-08-21 and four entries dated
2026-08-21 sit on the far side of it — they shipped in 0.7.0. The first entry below is the newest one
that tag could see, and it is the same instrument the 0.6 half of
[ROADMAP_HISTORY](ROADMAP_HISTORY_0_6.md) was cut with, so the two halves agree by construction rather
than by care.

This file is not edited again: an entry that needs a correction gets it in the live file, where a
reader will actually see it — the rule the pre-0.6 half already states.
...
[CHANGELOG_PRE_0_6.md](CHANGELOG_PRE_0_6.md).** The split is the 0.6.0 version bump, not a change of
