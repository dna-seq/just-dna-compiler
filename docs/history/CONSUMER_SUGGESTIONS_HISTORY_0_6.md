# Consumer suggestions — the 0.6 line
...
The middle half of [CONSUMER_SUGGESTIONS_HISTORY.md](../CONSUMER_SUGGESTIONS_HISTORY.md), split out on
2026-09-12 when 0.7.0 was cut and published: **S25–S61**, everything the 0.6 line answered. S1–S24,
S27 and S28 are one half further back in
[CONSUMER_SUGGESTIONS_HISTORY_PRE_0_6.md](CONSUMER_SUGGESTIONS_HISTORY_PRE_0_6.md); S62 onward stay
live.

**The contents list stays whole in the live file and covers all three halves**, because splitting an
index is how an item stops being findable — the same rule the 2026-08-17 split was made under, and the
reason `RM_TOC.md` exists.

**The boundary is the `v0.6.6` tag, not a date.** S61 is the highest `Sn` that tag could see, read off
`git show v0.6.6:docs/CONSUMER_SUGGESTIONS_HISTORY.md`; S62 arrived after it and was answered in the
0.7 line. The cut falls on a group heading, so no report is separated from the group that introduces
it.

**Every section here is a consumer's own words and stays byte-for-byte.** `triage-state.py` reads this
file as well as the live one — a well-formed archived section reads `current`, so anything else means a
marker went in wrong or did not survive a move.
