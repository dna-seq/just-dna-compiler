# Consumer suggestions — the items answered in the 0.5 line

The closed half of [CONSUMER_SUGGESTIONS_HISTORY.md](../CONSUMER_SUGGESTIONS_HISTORY.md), split out on
2026-08-17: **S1–S24, S27 and S28**, every one of them answered in 0.5.4 or earlier. The live history
file keeps what the 0.6 line answered, and [CONSUMER_SUGGESTIONS.md](../CONSUMER_SUGGESTIONS.md) is still
the open inbox.

The split is by **the release that answered an item**, not by the date it arrived — which is why S27
and S28 are here rather than beside S25 and S26. They were refiled on 2026-08-12 out of the deleted
round-1 thread, but what they were owed was written into the 0.5.4 docs, so they close the 0.5 line
rather than open the 0.6 one.

The rules that govern the live file govern this one. **The consumer's prose is moved byte-for-byte** —
verbatim into the history file once, and verbatim again into this one, fingerprints intact — and the
dangling `CONSUMER_FIELD_NOTES.md` link in the round-1 preamble is still left dangling on purpose. The
one-line contents list covering every item, this half included, stays whole in the live history file: a
contents list split in two is how an item becomes unfindable, which is the lesson `RM_TOC.md` carries.
...
**Status: consumer feedback / design input — not a shipped contract.** Same spirit as the round-1
thread `docs/CONSUMER_FIELD_NOTES.md`, deleted in `1f9dcd7` on 2026-08-12 and readable at `53f9260`:
illustrative asks framed to stay inside the
...
([RM17](ROADMAP_HISTORY_PRE_0_6.md#rm17--semver-on-moduleversion-coercing)), so the corpus-wide blocker is gone;
...
[RM44](ROADMAP_HISTORY_PRE_0_6.md#rm44--fully_resolved-answers-a-question-nobody-asked-it-and-prose-is-the-only-record-of-the-real-one),
