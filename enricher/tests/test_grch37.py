...
class TestRecoveryOutcomes:
    ...
    def test_an_allele_exact_query_recovers_exactly_one(self) -> None:
        recovery = recover_rsid("7", 140453136, ref="A", alts="T", client=_client(_overlap_handler()))
    ...
    def test_a_position_only_query_is_ambiguous_and_names_every_candidate(self) -> None:
        ...
        assert recovery.rsids == ["rs113488022", "rs121913227", "rs2128998293", "rs2128998296"]
...
class TestWhatRecoveryRefusesToMatch:
    ...
    def test_a_wrong_reference_allele_excludes_a_candidate(self) -> None:
        assert recover_rsid("7", 140453136, ref="C", client=_client(_overlap_handler())).rsids == []
    ...
    def test_an_alt_the_record_does_not_carry_excludes_it(self) -> None:
        ...
        assert (
            recover_rsid("7", 140453136, ref="A", alts="AT", client=_client(_overlap_handler())).rsids == []
        )
    ...
    def test_a_feature_from_another_assembly_is_ignored(self) -> None:
        ...
        assert recover_rsid("7", 140453136, client=_client(_overlap_handler(payload=foreign))).rsids == []
...
class TestTheWrongBuildDiagnosis:
    ...
    def test_a_single_base_match_is_reported_as_suggestive_only(self) -> None:
        result = diagnose_wrong_build([self._mismatch()], client=_client(self._handler(_HFE_GRCH37_BASE)))
    ...
    def test_a_multi_base_match_is_not_explained_by_chance(self) -> None:
        result = diagnose_wrong_build([self._mismatch("GATC")], client=_client(self._handler("GATC")))
...
class TestWhyItDidNotRun:
    ...
    def test_offline_says_so_and_makes_no_request(self) -> None:
        result = diagnose_wrong_build([RefMismatch("k", "6", 26093141, "G", "A")], offline=True)
    ...
    def test_a_bounded_pass_says_it_was_a_sample(self) -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return (
                ...
                if "/sequence/" in request.url.path
    ...
    def test_an_unbounded_pass_reports_no_sample(self) -> None:
        ...
        result = diagnose_wrong_build([RefMismatch("k", "6", 26093141, "G", "A")], client=_client(handler))
...
class TestGrouping:
    def test_a_whole_panel_of_one_cause_is_one_line(self) -> None:
        ...
        diagnoses = [BuildDiagnosis(f"k{n}", "6", 26093141 + n, "G", "single_base_match") for n in range(40)]
...
class TestPolitenessAndPacing:
    def test_every_request_goes_through_the_gate(self) -> None:
        ...
        """
        slept: list[float] = []
        client = Grch37Client(gate=PacingGate(interval=2.0, clock=lambda: 0.0, sleeper=slept.append))
