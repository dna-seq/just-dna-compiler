...
class FrequencyRow(BaseModel):
    ...
    rsid: str | None = Field(
        json_schema_extra=since("0.5.0"), default=None, description="dbSNP identifier, when known"
    )
    chrom: str | None = Field(
        json_schema_extra=since("0.5.0"), default=None, description="Chromosome without 'chr' prefix"
    )
    ...
    genome_build: str = Field(
        json_schema_extra=since("0.5.0"),
        default="GRCh38",
        description="Assembly the coordinate is in (the RM15 forward hook)",
    )
    ...
    allele_count: int | None = Field(
        ...
        default=None,
        ge=0,
        description="AC — observed copies of `alt` in this group",
    )
    allele_number: int | None = Field(
        json_schema_extra=since("0.5.0"),
        default=None,
        ge=0,
        description="AN — total called alleles in this group (the denominator)",
    )
    homozygote_count: int | None = Field(
        json_schema_extra=since("0.5.0"),
        default=None,
        ge=0,
        description="Individuals homozygous for `alt` in this group",
    )
    hemizygote_count: int | None = Field(
        json_schema_extra=since("0.5.0"),
        default=None,
        ge=0,
        description="Hemizygous calls (X/Y outside the PAR, and MT)",
    )
    ...
    vrs_id: str | None = Field(
        json_schema_extra=since("0.5.0"), default=None, description="GA4GH VRS allele id (`ga4gh:VA.…`)"
    )
    caid: str | None = Field(
        json_schema_extra=since("0.5.0"),
        default=None,
        description="ClinGen Allele Registry canonical allele id (`CA<digits>`)",
    )
    ...
    source: str | None = Field(
        json_schema_extra=since("0.5.0"),
        default=None,
        description="Which link filled this: gnomad|manual|reversed (open)",
    )
