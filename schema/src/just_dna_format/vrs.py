...
def in_pseudoautosomal_region(chrom: str | None, start: int | None, *, build: str = "GRCh38") -> bool | None:
...
def par_partner(chrom: str | None, start: int | None, *, build: str = "GRCh38") -> tuple[str, int] | None:
...
def sequence_location_digest(chrom: str | None, start: int, end: int, *, build: str = "GRCh38") -> str | None:
...
def validate_caid(value: str | None, field_name: str = "caid") -> str | None:
    ...
    if value is not None and not CAID_PATTERN.match(value):
        raise ValueError(f"{field_name} must be a ClinGen canonical allele id like CA125138, got: {value!r}")
    return value
