...
def test_the_missing_marker_is_its_own_reason_and_not_a_notation() -> None:
    ...
    A notation names a variant this grammar cannot spell — a gap a release may widen. `.` names no
    variant at all: the record is asserting that there is no alternate allele. Different claim,
    different consequence, so a different reason.

    `<DEL>` was this test's notation exemplar until RM5 landed in the same release and gave the five
    structural types a home, so it now answers `"symbolic"` — which is the same split one member over,
    and is asserted here beside the marker so neither can quietly re-absorb the other.
    """
    assert non_nucleotide_reason(MISSING_ALLELE) == "missing"
    assert non_nucleotide_reason("<DEL>") == "symbolic"
    ...
    # And the four stay four: a whitespace-padded cell is the same marker, not a fifth thing.
